...
export default function MenuItem({ sourceUrl, itemName }: MenuItemProps) {
  return (
    <div
      className="aspect-square w-30 md:w-34
        bg-amber-700/80
        bg-[radial-gradient(circle,var(--color-amber-900)_1.2px,transparent_1.2px)]
        bg-size-[15px_15px]
      flex flex-col justify-end rounded-2xl"
    >
      <div className="image-content h-2/3 w-full overflow-hidden">
        {sourceUrl ? (
          <img
            src={sourceUrl}
            alt={itemName ?? "Product image"}
            loading="lazy"
            className="w-full h-full object-cover rounded-t-2xl block"
          />
        ) : (
      ...
      <div
        className="text-data relative z-10
        h-1/3
